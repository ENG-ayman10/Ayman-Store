...
export function Header({ locale }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-neutral-200/80 dark:border-neutral-800/80 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md">
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 p-4 space-y-3 animate-in slide-in-from-top duration-200">
          <nav className="flex flex-col space-y-2 text-xs font-semibold">
            <Link
              href="/"
              onClick={() => setMobileMenuOpen(false)}
              className="p-2.5 rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-900 text-neutral-800 dark:text-neutral-200"
            >
              {isAr ? "الرئيسية" : "Home"}
            </Link>
            <Link
            ...
            <Link
              ...
              className="p-2.5 rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-900 text-neutral-800 dark:text-neutral-200"
            >
              {isAr ? "الملابس والأزياء" : "Fashion & Apparel"}
            </Link>
            <Link
              href="/track"
              onClick={() => setMobileMenuOpen(false)}
              className="p-2.5 rounded-xl bg-gold-50 dark:bg-gold-950/40 text-gold-800 dark:text-gold-300 font-bold flex items-center gap-2"
            >
              <Truck className="w-4 h-4" />
              <span>{isAr ? "تتبع حالة شحنتك" : "Track Your Order"}</span>
            </Link>
            <Link
              href="/admin"
              onClick={() => setMobileMenuOpen(false)}
              className="p-2.5 rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-900 text-neutral-500"
            >
              {isAr ? "لوحة الإدارة (Admin)" : "Admin Dashboard"}
            </Link>
          </nav>
